  ...
  <div class="flex min-h-screen bg-perplexity-bg" style="font-family: Inter, 'Noto Sans', '微软雅黑', sans-serif;">
    ...
    <nav class="side-nav">
      ...
      <div class="nav-btn-group">
        <el-button class="side-nav-btn" :class="{active: activeFunction === 'convert'}" text @click="activeFunction = 'convert'">
          <span class="nav-icon">📄</span>
          <span class="nav-label">智能识别</span>
        </el-button>
        <el-button class="side-nav-btn" :class="{active: activeFunction === 'match'}" text @click="activeFunction = 'match'">
          <span class="nav-icon">🔍</span>
          <span class="nav-label">智能匹配</span>
        </el-button>
        <el-button class="side-nav-btn" :class="{active: activeFunction === 'quote'}" text @click="activeFunction = 'quote'">
          <span class="nav-icon">💰</span>
          <span class="nav-label">生成报价单</span>
        </el-button>
        <el-button class="side-nav-btn" :class="{active: activeFunction === 'search'}" text @click="activeFunction = 'search'">
          <span class="nav-icon">🔎</span>
          <span class="nav-label">手动搜索</span>
        </el-button>
        <el-button ref="adminBtnRef" class="side-nav-btn" :class="{active: activeFunction === 'admin'}" text @click="toggleAdminMenu">
    ...
    <main class="main-content">
      <div v-if="activeFunction === 'convert'">
        ...
        <div class="upload-area mb-6">
          <input type="file" @change="handleConvertFileChange" accept=".xlsx,.xls" id="convert-file-input" style="display:none;" />
          <el-button type="primary" @click="triggerFileInput('convert')">选择Excel文件</el-button>
        </div>
        <div v-if="originalTableData.length > 0" class="split-view">
          ...
          <div class="converted-table">
            ...
            <div class="actions convert-actions button-flex-wrap">
              ...
              <el-button type="success" @click="exportConvertedData">导出结果</el-button>
              <el-button type="danger" @click="resetConvertData">清空数据</el-button>
              <el-button type="primary" @click="sendToMatch">智能匹配</el-button>
            </div>
          </div>
        </div>
      </div>
      <div v-if="activeFunction === 'match'">
        ...
        <div class="upload-area mb-6">
          <input type="file" @change="handleFileChange" accept=".xlsx,.xls" id="match-file-input" style="display:none;" />
          <el-button type="primary" @click="triggerFileInput('match')">选择Excel文件</el-button>
        </div>
        <div v-if="tableData.length > 0" class="table-area">
          ...
          <div class="actions">
            ...
            <el-button type="danger" @click="resetMatchData">清空数据</el-button>
            <el-button type="primary" @click="generateQuotes">生成报价单</el-button>
          </div>
        </div>
      </div>
      <div v-if="activeFunction === 'quote'">
        ...
        <div class="quote-actions">
          <el-button type="primary" @click="generateQuotes">更新报价单</el-button>
          <el-button type="success" @click="exportQuotes">导出报价单</el-button>
          <el-button type="danger" @click="resetQuotes">清空报价单</el-button>
        </div>
          ...
          <div class="admin-content">
            ...
            <div class="admin-modules">
              <div class="admin-module-card">
                <h3>费率管理</h3>
                <p>管理设备维护费率配置</p>
                <el-button type="primary" @click="openAdminSubPage('rate')">进入管理</el-button>
              </div>
              <div class="admin-module-card">
                <h3>服务级别管理</h3>
                <p>管理服务级别配置</p>
                <el-button type="primary" @click="openAdminSubPage('service_level')">进入管理</el-button>
              </div>
              <div class="admin-module-card">
                <h3>GPU价格管理</h3>
                <p>导入并管理GPU价格、费率与费用</p>
                <el-button type="primary" @click="openAdminSubPage('gpu_price')">进入管理</el-button>
              </div>
            </div>
          </div>
        </div>
    ...
    <div v-if="showSearchDialog" class="search-dialog" style="z-index: 2000;">
      <div class="search-dialog-content">
        <div class="search-header">
          <h3>手动搜索设备型号</h3>
          <el-button class="close-button" text @click="showSearchDialog = false">×</el-button>
        </div>
    ...
    try {
      ...
      if (response.data) {
        ...
        if (result.manufacturer) {
          ...
          manufacturerFilled = true;
        }
        // 无论是否有 matched_model，都尝试查详情接口补全厂商
        let detailModel = result.matched_model || item.model;
        try {
      const detailResp = await axios.get(`${API_URL}/devices/search/?model=${encodeURIComponent(detailModel)}&source=${encodeURIComponent(dataSource.value)}`);
          if (detailResp.data && Array.isArray(detailResp.data.data) && detailResp.data.data.length > 0 && detailResp.data.data[0].manufacturer) {
            tableData.value[index].manufacturer = detailResp.data.data[0].manufacturer;
            manufacturerFilled = true;
          }
        } catch {}
        // 若仍未补全，保持原值
...
async function handleFileChange(event) {
  ...
  reader.onload = async (e) => {
    try {
      ...
      // 调用批量匹配API
      try {
        const matchRequests = items.map(item => ({
          manufacturer: item.manufacturer,
          model: item.model,
          category: item.category,
        ...
        const response = await axios.post(`${API_URL}/bulk-match/`, { items: matchRequests })
        
        // 更新匹配结果
        if (response.data && response.data.length === items.length) {
          for (let i = 0; i < items.length; i++) {
            ...
            items[i].deviceCategory = result.device_category || '' // 设置设备三级分类
            
            // 同时下载该型号的所有可能匹配
            try {
              // 优先搜索匹配到的型号，以获取匹配型号详情
              let searchModel = items[i].model
              if (items[i].matchedModel) {
                // 如果有匹配的型号，优先搜索匹配型号
                searchModel = items[i].matchedModel
              }
              const searchResponse = await axios.get(`${API_URL}/devices/search/?model=${encodeURIComponent(searchModel)}&source=${encodeURIComponent(dataSource.value)}`)
              // 保存所有可能的匹配结果，以便后续手动选择
              items[i].possibleMatches = searchResponse.data
              
              // 如果当前没有匹配结果，或者匹配的型号不在结果中，再搜索原始型号
              if (items[i].matchedModel && !searchResponse.data.some(m => m.model === items[i].matchedModel) && searchModel !== items[i].model) {
                const originalSearchResponse = await axios.get(`${API_URL}/devices/search/?model=${encodeURIComponent(items[i].model)}&source=${encodeURIComponent(dataSource.value)}`)
                // 合并两次搜索结果，去重
                const combinedResults = [...searchResponse.data]
                for (const item of originalSearchResponse.data) {
                  if (!combinedResults.some(m => m.model === item.model)) {
                    combinedResults.push(item)
                  }
                }
                items[i].possibleMatches = combinedResults
              }
            } catch (error) {
        ...
        for (const item of items) {
          ...
          item.price = matchRate >= 50 ? Math.random() * 10000 : undefined
        }
      }
