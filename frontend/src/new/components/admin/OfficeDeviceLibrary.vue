<template>
  <div class="office-device-library">
    <!-- 数据表格容器 -->
        ...
        <div class="dialog-body">
          <div class="form-grid">
            <div class="form-group">
              ...
              <input v-model="formData.manufacturer" placeholder="例如: 联想" />
            </div>
            <div class="form-group">
              <label>设备系列</label>
              <input v-model="formData.device_series" placeholder="例如: ThinkPad" />
            </div>
            <div class="form-group">
              <label>设备型号 <span class="required">*</span></label>
              <input v-model="formData.model_number" placeholder="例如: X1 Carbon" />
            </div>
            <div class="form-group">
              <label>业务场景</label>
            ...
            <div class="form-group">
              <label>一级分类</label>
              <input v-model="formData.primary_category" placeholder="例如: 笔记本" />
            </div>
            <div class="form-group">
              <label>二级分类</label>
              <input v-model="formData.secondary_category" placeholder="例如: 商务本" />
            </div>
            <div class="form-group">
              <label>三级分类</label>
              <input v-model="formData.tertiary_category" placeholder="例如: 轻薄本" />
            </div>
            <div class="form-group">
              <label>设备档次</label>
            ...
            <div class="form-group full-width">
              <label>整机价格 (¥)</label>
              <input v-model.number="formData.device_price" type="number" step="0.01" placeholder="例如: 5800.00" />
            </div>
            <div class="form-group full-width">
              <label>备注</label>
...
async function batchDelete() {
  try {
    ...
    const promises = selectedDevices.value.map(id => axios.delete(`${API_URL}/office-devices/${id}`))
...
async function applyBatchEdit() {
  ...
  try {
    ...
    const promises = selectedDevices.value.map(id =>
      axios.put(`${API_URL}/office-devices/${id}`, {
...
async function loadDevices() {
  ...
  try {
    ...
    const url = `${API_URL}/office-devices/`
    ...
    console.log('[OfficeDeviceLibrary] Loading devices from:', url + '?' + params.toString())
    const response = await axios.get(url + '?' + params.toString())
    console.log('[OfficeDeviceLibrary] Response:', response.data)
    devices.value = response.data?.data || []
    pagination.value.total = response.data?.total || 0
  } catch (error: any) {
    console.error('[OfficeDeviceLibrary] Failed to load devices:', error)
...
async function loadFilterOptions() {
  try {
    const [manufRes, seriesRes, scenariosRes, gradesRes] = await Promise.all([
      axios.get(`${API_URL}/office-devices/manufacturers`),
      axios.get(`${API_URL}/office-devices/device-series`),
      axios.get(`${API_URL}/office-devices/business-scenarios`),
      axios.get(`${API_URL}/office-devices/device-grades`)
    ])
...
async function saveDevice() {
  ...
  try {
    ...
    if (editingDevice.value) {
      await axios.put(`${API_URL}/office-devices/${editingDevice.value.id}`, data)
      ElMessage.success('设备更新成功')
    } else {
      await axios.post(`${API_URL}/office-devices/`, data)
...
async function deleteDevice(id: number) {
  try {
    ...
    await axios.delete(`${API_URL}/office-devices/${id}`)
