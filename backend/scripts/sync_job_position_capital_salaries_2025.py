#!/usr/bin/env python3
"""
增量同步 2025 驻场技术序列的 7 个省会城市薪资。

新版工作簿中，下列城市的 92 个技术序列薪资与已有城市档位完全一致。
脚本按岗位 ID 复制已有档位，只新增或更新目标城市，不清空岗位表。
...
def sync_capital_salaries(apply_changes: bool) -> int:
    db = SessionLocal()
    try:
        source_cities = {source for _, source in TARGET_CITY_PROFILES.values()}
        source_rows = (
            db.query(JobPositionSalary)
            .join(JobPosition, JobPosition.id == JobPositionSalary.position_id)
            .filter(
                JobPosition.sequence_type == "技术序列",
        ...
        for source_city in source_cities:
            ...
            if count != EXPECTED_TECHNICAL_POSITION_COUNT:
                print(
                    f"源城市 {source_city} 技术序列数据不完整："
                    f"期望 {EXPECTED_TECHNICAL_POSITION_COUNT}，实际 {count}"
                )
                db.rollback()
                return 1

        technical_position_ids = [
            row[0]
            for row in (
                db.query(JobPosition.id)
                .filter(JobPosition.sequence_type == "技术序列")
                .order_by(JobPosition.id)
                .all()
            )
        ]
        if len(technical_position_ids) != EXPECTED_TECHNICAL_POSITION_COUNT:
            print(
                f"技术序列岗位数异常：期望 "
                f"{EXPECTED_TECHNICAL_POSITION_COUNT}，实际 {len(technical_position_ids)}"
            )
            db.rollback()
            return 1
        ...
        updated = 0
        unchanged = 0
        for target_city, (province, source_city) in TARGET_CITY_PROFILES.items():
            existing = {
                row.position_id: row
                for row in (
                    db.query(JobPositionSalary)
                    .filter(JobPositionSalary.city == target_city)
                    .all()
                )
            }
            for position_id in technical_position_ids:
        ...
        expected_total = EXPECTED_TECHNICAL_POSITION_COUNT * len(TARGET_CITY_PROFILES)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="增量同步 2025 驻场技术序列省会城市薪资"
    )
    parser.add_argument(
        "--apply",
        action="store_true",
